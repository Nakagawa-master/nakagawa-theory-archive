...
JA_IMPERATIVE=re.compile(r"(?:してはならない|してはいけない|してはいけません|しなければならない|必ず保持|保持すべき|追加すべき|削除すべき)")
EN_LINE_START=re.compile(r"^\s*(?:[-*]\s*)?(?:Preserve|Do not|Don't|Keep|Retain|Avoid|Return|Never|Always|Must|Should)\b",re.I)
EN_CREATOR=re.compile(r"\b(?:this|the) (?:official )?(?:derivative|index|public reading)\b.{0,180}\b(?:must|should|shall|do not|does not|preserve|retain|keep|add|generate|limit)\b",re.I)
EN_AI=re.compile(r"\b(?:third[- ]party )?AI(?:s)?\b.{0,180}\b(?:must|should|shall|do not|preserve|retain|keep|add|generate|limit)\b",re.I)
ZH_CREATOR=re.compile(r"(?:本公开读解|本衍生物|官方衍生物|本索引|第三方AI).{0,180}(?:不得|必须|应该|应当|不应|保持|保留|生成|添加|限定|删除)")
FAQ_BAD=re.compile(r"^### Q\d+\..*(?:この公開読解|派生物|公開派生物|追加して|保持すべき|完結して|削除|作って|設定して|すべき|してよい|してもよい|してはいけない)")
...
def main():
    ...
    for i in range(1,31):
        ...
        for name in SURFACES:
            ...
            if b<FLOORS[name]:
                issues.append((od,name,"underfloor",f"bytes={b} floor={FLOORS[name]}"));counts["underfloor"]+=1
            ...
            for ln,line in enumerate(text.splitlines(),1):
                ...
                if name in ("README.md","human-entry.md","faq.md","ai-index.md","derivative-ledger.md"):
                    ...
                    elif JA_IMPERATIVE.search(line): bad="JA imperative"
            ...
            if not all(link in text[-800:] for link in FOOTER_LINKS):
        ...
        # FAQ Q1-Q30 exactly once, ordered.
        faq=files["faq.md"]
        ...
        # AI fixed headings in all three languages: English labels are literal benchmark labels.
        for name in ("ai-index.md","en-ai-index.md","zh-ai-index.md"):
            tx=files[name]; got=re.findall(r"^## (.+?)\s*$",tx,re.M)
            # Parent/identity headings precede fixed 13; check each required fixed heading exists exactly once and order.
            pos=[]
        ...
        # Falsification: README + JA AI must be same source-derived body; no derivative meta-rule.
        ...
        for name in ("README.md","ai-index.md","en-ai-index.md","zh-ai-index.md"):
            ...
            fs=section(tx,head)
            if any(x in fs for x in ["派生物側","この公開読解","this derivative","the derivative","本公开读解","本衍生物"]):
                issues.append((od,name,"falsification","derivative-created/meta wording in falsification"));counts["falsification"]+=1
        # No famous media name in derivatives.
