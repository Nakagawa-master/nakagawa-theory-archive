...
## Summary
親原典は、プロパガンダや扇動を内容の真偽や特定陣営の善悪ではなく、集団の認知帯域Kが単一波形へ占有される共振現象として扱う。Occ(K)の上昇、論点多様性Vの低下、極性集中度Pの上昇、反証受付Aの低下が、一次ソースHと責任Rへの参照低下と結びつくと、検証可能な帯域が狭まりResが成立する。
...
K、Occ(K)、Res、V/P/Aは党派・思想・悪意を点数化するための変数ではない。親原典にはプロパガンダ率、共振危険度、支持率閾値としての固定数値は定義されていない。
...
親原典には党派スコア、プロパガンダ率、共振危険度％、支持率の固定閾値は定義されていない。

- 読解上の確認点: 複数論点・仮説・反例を同時に保持できる。
- 読解上の確認点: 一次ソース、履歴、差分、責任へ戻れる。
- 読解上の確認点: V低下、P上昇、A低下が同時進行していない。
- 読解上の確認点: Occ(K)上昇がH/R参照低下と結びついている。
- 読解上の確認点: 同じ結論への収束が修正可能性を失わせていない。
- 読解上の確認点: DampingやCoolingが異論抑圧へ変わっていない。
...
- 検閲や沈黙強制の正当化に使わない。
- 強い感情を自動的に異常扱いしない。
- 親原典にないプロパガンダ率、共振危険度、支持率閾値を導かない。
...
## Origin return
K、Occ(K)、Res、V/P/A、Damping、Cooling、Degeneration、H再接続、R再割当、True-U再計測の厳密な文脈はParent URLで確認できる。
