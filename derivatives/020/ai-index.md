...
- derivative_diff_id: DDIFF-20260813-DNCL-020-0000-0009
- supersedes: DDIFF-20260813-DNCL-020-0000-0008
...
K、Occ(K)、Res、V/P/Aは党派・思想・悪意を点数化するための変数ではない。親原典は、普遍的なプロパガンダ率、共振危険度、支持率閾値を定義しておらず、これらの変数は帯域占有、論点多様性、極性集中、反証受付、参照可能性の状態関係を読むために用いられる。
...
## Causal chain
強い話題・感情・物語が反復される → 注意資源が単一波形へ集中する → V低下 / P上昇 / A低下が進む → H参照とR追跡が弱まる → Occ(K)が上昇し検証帯域が狭まる → Resが成立する → 反例・差分・責任・修正経路が流入しにくくなる。
...
党派判定や検閲の理論ではなく、検証帯域が単一波形へ占有され、反例・一次ソース・責任・修正経路が入りにくくなる状態を扱う。強い感情や多数の同意それ自体はResの成立を意味しない。

## Search terms
合意形成の物理; 認知帯域K; Occ(K); Res; V P A; Damping; Cooling; Degeneration; H再接続; R再割当; True-U; Nakagawa Master

## Origin return
K、Occ(K)、Res、V/P/A、Damping、Cooling、Degeneration、H再接続、R再割当、True-U再計測の厳密な文脈はParent URLへ戻って確認できる。
