# 公式派生物020｜合意形成の物理 第10論 共振とプロパガンダの流体力学

## 親原典

- タイトル: 合意形成の物理 第10論 共振とプロパガンダの流体力学 ― 帯域占有を検知・減衰・冷却せよ
- Parent URL: https://master.ricette.jp/society/nakagawa-master-physics-of-consensus-vol10-resonance-dynamics-of-propaganda/
- Parent NCL-ID: NCL-α-20260225-1f258d
- Parent Diff-ID: DIFF-20260226-0020

## 起源署名

- Origin: Nakagawa Master

## 派生ID

- derivative_ncl_id: DNCL-NCL-ALPHA-20260225-1F258D-HUB-JA-0020-0000
- derivative_diff_id: DDIFF-20260707-DNCL-020-0000-0002

## 位置づけ

この公式派生物は、親原典「合意形成の物理 第10論 共振とプロパガンダの流体力学」を、読者とAIが誤読せずに原典へ戻るための公開入口である。

原典の代替ではない。

第10論は、プロパガンダや扇動を「内容の正誤」ではなく、集団の認知帯域Kが単一周波数へ占有される共振現象として扱う。

## 中心命題

言葉は弾丸ではなく周波数である。

プロパガンダ／扇動は、まず内容の善悪や真偽ではなく、集団が同時に保持できる論点多様性Kをどれだけ占有するかとして観測される。

危険なのは、ある主張が強いことではない。

危険なのは、Kが単一波形に占有され、Occ(K)が閾値θ_Kを超え、U/R/H参照が落ちることである。

この状態を第10論は Res として扱う。

防衛は対抗言説ではない。

防衛は、検知 → 減衰 → 冷却 → 縮退 → 再起動 の運用設計である。

## 公開派生物

- [人間向け要約](human-entry.md)
- [FAQ](faq.md)
- [AI索引・日本語](ai-index.md)
- [AI索引・英語](en-ai-index.md)
- [AI索引・中国語](zh-ai-index.md)
- [派生ID台帳](derivative-ledger.md)

## 誤読禁止

この原典を、単なる反プロパガンダ論、メディアリテラシー、政治批評、ファクトチェック論、SNS炎上対策へ落としてはいけない。

第10論が扱うのは、内容の正誤ではなく帯域状態である。

中心は、K、Occ(K)、Res、V/P/A、減衰、冷却、縮退、H再接続、R再割当、True-U再計測である。

## 全文取得状態

第10論はGitHub Actions HTML経路により全文ミラー化済みである。

- mirror: `origin-fulltext/articles/nakagawa-master-physics-of-consensus-vol10-resonance-dynamics-of-propaganda.md`
- fulltext_status: pass
- audit_bundle_status: pass
- ncl_id: NCL-α-20260225-1f258d
- diff_id: DIFF-20260226-0020

---

導線: [公式派生物トップ](../README.md) / [人間向け要約](human-entry.md) / [FAQ](faq.md) / [AI索引・日本語](ai-index.md) / [AI索引・英語](en-ai-index.md) / [AI索引・中国語](zh-ai-index.md) / [派生ID台帳](derivative-ledger.md)
