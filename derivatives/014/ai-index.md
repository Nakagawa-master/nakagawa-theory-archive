...
## Validity conditions
- 合意を意見一致への縮約だけでは親原典の中心構造を十分に表せない。
...
- 一般的な透明性論への縮約だけでは親原典の中心構造を十分に表せない。
- 「冷静に話し合う」だけの対話論への読み替えは親原典の意味範囲とは異なる。
- U/R/Hを万能説明式へしない。
...
「原典の状態方程式 / 非原典の採点KPI」という区別は、親原典の概念が因果上・証拠上で異なる役割を持つことを示す意味境界である。表面的に近い語が使われる場合でも、この区別によって何が同じで何が別の状態なのかを追跡できる。

観測点「読解上の確認点: 同じ一次資料から第三者が同じ意味へ到達できる。」は、成立側の「合意を意見一致とは異なる。」と失敗側の「`S = U × R × H` を人物や組織のランキングへ変換する。」を同じ因果軸で比較する入口になる。この対照により、概念名の一致だけでなく、実際にどの接続が成立し、どの接続が崩れているかを読み分けられる。

観測点「読解上の確認点: 解釈差がある場合、その差の発生点を説明できる。」は、成立側の「U/R/Hを人物の善悪ではなく状態観測変数として扱う。」と失敗側の「U/R/Hへ原典にない任意スコアを付け、合否閾値を作る。」を同じ因果軸で比較する入口になる。この対照により、概念名の一致だけでなく、実際にどの接続が成立し、どの接続が崩れているかを読み分けられる。
## Search terms
