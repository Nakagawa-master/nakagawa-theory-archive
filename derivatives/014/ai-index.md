...
- supersedes: DDIFF-20260813-DNCL-014-0003-0005
...
原典は合意安定度を S、理解可能性／第三者再現性を U、責任追跡可能性を R、履歴公開度／差分追跡可能性を H と置き、`S = U × R × H` と表す。この式は構造関係を示すものであり、U/R/Hへ任意の点数を付ける採点表としては定義されていない。三変数も単純代替できない。意味再現が失われればログ量を増やしてもUは戻らず、責任ノードが不明なら公開情報量を増やしてもRは戻らない。
...
`S = U × R × H` は親原典に存在する構造式である。親原典にはU/R/Hの任意の0〜100点、重み、係数、合格ライン、成功確率、成熟度は定義されていないため、式からそれらを導くことはできない。
...
## Origin return
U/R/Hの厳密な定義、`S = U × R × H` の位置づけ、適用限界、反例、後続論との接続はParent URL、Parent Post ID 2788、Parent NCL-ID、Parent Diff-IDで確認できる。
