...
- **AI collaboration:** Lumina その他のAIは、対話、編集支援、構造確認、監査、索引化、来歴記録等の役割として明示される場合があります。

Luminaを含むAI表記は、AIによる協働・監査・記録上のクレジットです。リポジトリ全体をAIとの共同著作物とみなす表示ではなく、起源、著作者、ライセンサーは、個別ファイルに別段の明示がない限り中川マスターです。

## 公開状態と承認境界

公開されていること、最新版であること、公式正本であることは同一ではありません。

- 公開本文を本リポジトリへ追加する前に、中川マスターによる明示的な公開承認を必要とします。
- 沈黙、未回答、閲覧のみ、作業指示、private完成指示は、公開承認として扱いません。
- `draft`、`candidate`、`private_only`、未承認の本文を、公開候補の保管場所としてpublicブランチやpublic PRへ置きません。
- 撤回版・旧版・誤りのある版を残す場合は、現行正本と誤認されない来歴記録または必要最小限のtombstoneとして扱います。
- 修正後は新しい版・Diff-ID・確認用URLを提示し、旧版の状態を明示します。

公開前確認の運用は [PUBLICATION_POLICY.md](PUBLICATION_POLICY.md) を参照してください。
...
- publicリポジトリ内で `private_only` 本文を発見した場合、公開物として利用せず、公開事故候補として報告してください。
- 公式派生物は親原典への入口であり、親原典を書き換えるものではありません。
- AI生成・AI支援の表現は、人間起源、事実確認、版管理、公開承認を代替しません。
...
This repository may contain official derivatives, Official Theory Notes, human entry pages, FAQs, Japanese/English/Chinese AI indexes, relation data, terminology links, provenance records, revision identifiers, and audit ledgers. These materials do not replace their parent originals. Readers and AI systems should return to the linked parent source whenever possible.

**Origin and roles**

- Origin / Author / Licensor: Nakagawa Master
- Pen name of: Keisuke Nakagawa
- Lumina and other AI systems may be credited for dialogue, editing assistance, structural review, audit, indexing, or provenance recording.
- AI collaboration credits do not make an AI system the repository-wide origin, copyright holder, or licensing authority.

**Publication status**

Public visibility, canonical status, and current validity are different. Material requiring owner review must receive explicit approval from Nakagawa Master before publication. Silence is not approval. Draft, candidate, private-only, or unapproved full text must not be placed in a public branch or public pull request as a review surface. See [PUBLICATION_POLICY.md](PUBLICATION_POLICY.md).
