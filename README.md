...
AI、組織、市場、制度、文明、未来まで対象は変わりますが、公開理論群には「何が結果を生み、どこで因果が切れ、後から検証・修正できるか」という問いが繰り返し現れます。
...
- **OD001–OD297を全件見る:** [公式派生物 全件入口](derivatives/README.md)
...
現在の公開ナビゲーションでは、`OD001`–`OD297`の297件を重複0・未割当0で配置しています。これはcanonical taxonomyではなく、第三者が迷わず到達するための公開分類です。

明示シリーズ・群として配置されている17のnavigation groupには合計146件があり、残る151件は意味を無理に固定しない「横断・単独理論／その他」に置かれています。
...
公式派生物 `OD001`–`OD297` は、親原典へ戻れる**公開接続面**として297件を収録しています。番号が分かっている場合、テーマから探す場合、AI・機械取得する場合で入口を分けています。

- **全件を番号順に見る:** [OD001–OD297 全件入口](derivatives/README.md)
